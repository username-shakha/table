...
export const useOverlayState = <T, P>(defaultIsOpen: boolean = false): OverlayStateManagerProps<T, P> => {
    ...
    const resolve = useCallback((val: T) => {
        if (!promiseRef.current) throw new Error('"resolve" cannot be used when "isOpen" is false.')
    ...
    const close = useCallback(() => {
        if (!promiseRef.current) throw new Error('"close" cannot be used when "isOpen" is false.')
